...
if(empty($_POST['documento']) || empty($_POST['tipo_documento']) || empty($_POST['nombre']) || empty($_POST['apellidos']) || empty($_POST['correo']) || empty($_POST['clave']) || empty($_POST['direccion']) || empty($_POST['celular']) || empty($_POST['rol']) || empty($_POST['estado'])){
...
$sql= "INSERT INTO clientes(documento, tipo_documento, nombre, apellidos, correo, clave, direccion, celular, rol, estado) VALUES (?,?,?,?,?,?,?,?,?,?)";
$stmt= mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "isssssssss", $documento, $tipo_documento, $nombre, $apellidos, $correo, $clave, $direccion, $celular, $rol, $estado);
mysqli_stmt_execute($stmt);
